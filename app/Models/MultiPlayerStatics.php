...
use DB;
class MultiPlayerStatics extends Model {
    protected $fillable = [
       
    ];
    
    protected $table    = 'multi_player_statics';
    protected $hidden   = [];
    ...
    public static function add_multi_player_statics($param='')
    {
...
//                       dd($e->getMessage());
    ...
    public static function update_multi_player_statics($param='')
    {
//             dd($param);
            try {
                    $game=self::where('user_id',$param['user_id'])->first();
                     if(is_null($game)){
                         
                            return \General::error_res("Game  Not Found");
                     }

                    $game->played=$param['played']; 
                    $game->won=$param['won'];
                    $game->tie=$param['tie'];
                    $game->lost=$param['lost'];
                    $game->total_score=$param['score'];
...
//                  dd($e->getMessage());
    ...
    public static function filter_multi_player_statics($param){
